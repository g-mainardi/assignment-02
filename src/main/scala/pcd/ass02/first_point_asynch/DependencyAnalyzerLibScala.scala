...
object DependencyAnalyzerLibScala {
  ...
  def getPackageDependencies(source: File): Future[PackageDepsReport] = vertx executeBlocking { () =>
    source match
      case s if !s.isDirectory => throw IllegalArgumentException("This [" + source.toPath + "] is not a package")
  ...
  def getProjectDependencies(source: File): Future[ProjectDepsReport] = vertx executeBlocking { () => source match
    case s if !s.isDirectory => throw IllegalArgumentException("Not a dir")
    case s                   =>
